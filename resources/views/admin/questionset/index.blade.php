...
@include('admin.note.add')
    <div class="modal fade editnote" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#edu-data-table').DataTable({
                ...
                bInfo: false,
                bAutoWidth: false,
                ajax: "{{ route('admin.json.questionset') }}",
                columns: [
                    {
                        data: 'count',
                    ...
                    {data: 'file', name: 'file',
                        render: function(data, type, row) {
                                return '<a href=' + row.note_file + ' class="btn-chnage-status btn btn-sm btn-default text-primary btn-outline-primary" target="_blank"> View Note </a>';
                        }
                    },
    ...
    <script>
        $(document).on('submit', '#eduForm', function (e) {
            ...
            var myUrl = "{{ route('admin.store.questionset') }}";
    ...
    <script>
        $(document).on("click", ".btn-edit-row", function (e) {
            ...
            var tempEditUrl = "{{ route('admin.edit.questionset', ':id') }}";
    ...
    <script>
        $(document).on('submit', '#eduFormUpdate', function (e) {
            ...
            var myUrl = "{{ route('admin.update.questionset') }}";
    ...
    <script>
        $(document).on('click', '.btn-chnage-status', function (e) {
            e.preventDefault();
            var id = $(this).attr("data-id");
            var tempDeleteUrl = "{{ route('admin.changestatus.questionset', ':id') }}";
    ...
    <script>
        $(document).on('click', '.btn-delete-row', function (e) {
            e.preventDefault();
            var id = $(this).attr("data-id");
            var tempDeleteUrl = "{{ route('admin.delete.questionset', ':id') }}";
            tempDeleteUrl = tempDeleteUrl.replace(':id', id);
    ...
    <script>
        $(document).on("change", ".selectforchild", function (e) {
            ...
            var tempResponseUrl = "{{ route('admin.change.questionsetcontent') }}";
